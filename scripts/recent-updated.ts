...
export async function generateRecentUpdated() {
  ...
  const names = uniq(
    logs.all
      .map((i) => {
        const name = /^\w+!?\((\w+)\)!?:/.exec(i.message)?.[1]
        if (name && functionNames.includes(name)) {
          if (!hashMaps[name]) hashMaps[name] = i.hash
          return name
        }
        return null
      })
    ...
    news
      .map((name) => {
        ...
        if (fn) {
          ...
          return {
            ...fn,
            description: `${fn.description} ([\`${hash.slice(
              0,
              5
            )}\`](https://github.com/kongying-tavern/design/commit/${hash}))`,
          }
        }
        return null
      })
...
`
  ...
  await fs.writeFile(
    resolve(__dirname, '../packages/recent-updated.md'),
    md,
    'utf-8'
  )
}
