...
const DOCS_URL = 'https://kongying-tavern.github.io'
const GITHUB_BLOB_URL =
  'https://github.com/kongying-tavern/design/blob/main/packages'
...
export function getFunctionHead(pkg: string, name: string) {
  let head = packages.find((p) => p.name === pkg)!.addon
    ? `available in add-on [\`@kongying-tavern/${pkg}\`](/${pkg}/README)`
    : ''
...
export function getCategories(functions: KyDesignFunction[]): string[] {
  return uniq(
    functions
      .filter((i) => !i.internal)
      .map((i) => i.category)
...
export async function updateImport({ packages, functions }: PackageIndexes) {
  for (const { name, dir, manualImport } of Object.values(packages)) {
    ...
    if (name === 'core') content += "\nexport * from '@kongying-tavern/shared'"
...
export async function updateFunctionsMD({
  ...
  let mdFn = await fs.readFile('packages/packages.md', 'utf-8')
  ...
  await fs.writeFile('packages/packages.md', mdFn, 'utf-8')
}
...
export async function updateFunctionREADME(indexes: PackageIndexes) {
  ...
  for (const fn of indexes.functions) {
    ...
    if (hasTypes)
      readme = replacer(
        readme,
        await getFunctionFooter(fn.package, fn.name),
        'FOOTER',
        'tail'
      )
...
export async function updatePackageJSON(indexes: PackageIndexes) {
  ...
  for (const {
    ...
    packageJSON.author = author || 'JiaZeng<https://github.com/jiazengp>'
    packageJSON.bugs = {
      url: 'https://github.com/kongying-tavern/design/issues',
    }
    packageJSON.homepage =
      name === 'core'
        ? 'https://github.com/kongying-tavern/design#readme'
        : `https://github.com/kongying-tavern/design/tree/main/packages/${name}#readme`
    ...
    for (const key of Object.keys(packageJSON.dependencies)) {
      if (key.startsWith('@kongying-tavern/'))
